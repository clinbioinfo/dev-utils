...
use constant DEFAULT_COMMIT_AND_PUSH => TRUE;
...
my $results = GetOptions (
    ...
    'indir=s'                        => \$indir,
    'outdir=s'                       => \$outdir,
    'git-comment-file=s'             => \$git_comment_file,
    'git-asset-list-file=s'          => \$git_asset_list_file,    
    'admin-email-address=s'          => \$admin_email_address,
    'commit-and-push'                => \$is_commit_and_push,
    );
...
if (defined($git_comment_file)){
    $git_manager->setCommitCommentFile($git_comment_file);
}

if (defined($git_asset_list_file)){
    $git_manager->setCommitAssetListFile($git_asset_list_file);
}
...
sub checkCommandLineArguments {
    ...
    if (!defined($is_commit_and_push)){
        ...
        $is_commit_and_push = DEFAULT_COMMIT_AND_PUSH;

        printYellow("--commit-and-push was not specified and therefore was set to default '$is_commit_and_push'");
    }
    ...
    if (!defined($admin_email_address)){
        ...
        printYellow("--admin-email-address was not specified and therefore was set to default '$admin_email_address'");
    }
