...
my $manager = DevelopmentUtils::MongoDB::Manager::getInstance(
    ...
    indir                => $indir,
    test_mode            => $test_mode,
    mongodb_service_file => $mongodb_service_file
    );

if (!defined($manager)){
    $logger->logdie("Could not instantiate DevelopmentUtils::MongoDB::Manager");
}
...
=head1 NAME
 ...
 mongodb_helper.pl - Perl script for facilitate execution of MongoDB-related activities and actions


=head1 SYNOPSIS

 perl util/mongodb_helper.pl
...
=item B<--outdir>
  ...
  A default value is assigned /tmp/[username]/mongodb_helper.pl/[timestamp]/
...
=item B<--logfile>
  ...
  Default is set to [outdir]/mongodb_helper.pl.log
